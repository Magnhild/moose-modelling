...
Quadrat_name <- as.factor(first_data$Quadrat_name)

library(lme4)

mixed.model1 <- glmer(moose ~   biomass + area + house + road + bilberry + class + psy + FSAV + veg+ wolf + dominant + trees + acc + hunting + accpsy + species + (1|Site) + (1|Quadrat_name), data=first_data, family = poisson) # link function, but no p value
anova(mixed.model1)
...
mixed.model8 <- glmer(moose ~   biomass + area + class + psy + veg+  trees + acc + (1|Site/Quadrat_name),  family = poisson) # link function, but no p value
anova(mixed.model8)
print(mixed.model8, correlation=TRUE)
